...
############################## IMPORTANT ##############################
# This program is nearly identical to talkNano.py with the exception that
# it returns the output of the command fed to readNano.py, allowing the
# user to determine the status of any communication sent to the TPC Pi
#######################################################################
...
#        print(format)
    ...
    # print(command)
    # After assembling the command structure with prefix, commands and arguments,
    # send into readNano to send command to chip
    e = readNano(command)

    # extract bits from returned bytes
