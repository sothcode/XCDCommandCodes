...
# closed loop to new random point and record the arrived position
# repeat as many times as necessary
def testCLAccuracy( axis, numPoints=1 ):

    # check numPoints greater than 0
    if numPoints <= 0:
        print("CLAccuracyTest.py needs a nonzero number of times to move.")
        return

    # check status, then initialize proper variables
    status=readback(ADDR['STATUS'])
    if status!=0:
        print("CLAccuracyTest.py initilization failed. Please check status.")
        return

    # change to input axis (and check if axis exists)
    s = changeAxis(axis)
    if not s:
        print("CLAccuracyTest.py: did not change axis properly.")
        return False
    ...
    REPORTFILE = filename + "_CLtest.txt"
    ...
    for i in range(numPoints):
        ...
        didGoTo, position = goto(axis, destination, False)
        if not didGoTo:
            print("CLAccuracyTest.py: goto move iteration ", i, " failed.")
            if position == 0:
                status = 99
            status = readback(ADDR['STATUS'])
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 2:
        ...
        try:
            ...
            print("   ./CLAccuracyTest.py laser_name loop_number(int)")
            sys.exit()
        testCLAccuracy(axis, numPts)
    else:
        print("NOT EXECUTED. Wrong number of arguments.  Correct usage is:")
        print("   ./CLAccuracyTest.py laser_name loop_number")
        sys.exit()