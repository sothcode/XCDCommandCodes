...
lbRel=hardstop1-position
hbRel=hardstop2-position
...
if(abs(lbRel)>matchTolerance):
    print("FAIL.  home and lowbound are not the same?  Motor likely jammed.  span:%s. lbrel:%s hbrel:%s home:%s%s%s."%(span,lbRel,hbRel, homeRel,"(Real)"*homeIsReal,"(NotFound)"*(not homeIsReal)))
    print("aborting.  Db will not be updated.")
    sys.exit()
