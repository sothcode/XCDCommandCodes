...
def gotoThetaL( whereToGo ):
    ...
    if status==STAT['READY']:
        print("SUCCESS. gotoThetaL complete.  status: %s (%s) position:%1.5f lb:%1.5f hb:%1.5f"%(status,_reverseLookup(STAT,status),position,lb,hb))
        return True, position
    else:
        print("FAIL. gotoThetaL failed.  status: %s (%s) position:%1.5f lb:%1.5f hb:%1.5f"%(status,_reverseLookup(STAT,status),position,lb,hb))
